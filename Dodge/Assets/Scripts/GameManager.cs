...
public class GameManager : MonoBehaviour
{
    ...
    void Update()
    {
        if(!isGameover)
        {
            ...
            if(surviveTime < 5f && spawnerCounter == 0)
            {
                GameObject bulletSpawner = Instantiate(bullerSpawnerPrefab, bulletSpawners[spawnerCounter], Quaternion.identity);
                bulletSpawner.transform.parent = level.transform;
                bulletSpawner.transform.localPosition = bulletSpawners[spawnerCounter];
                level.GetComponent<Rotator>().rotationSpeed += 15f;
                spawnerCounter++;
            }
            else if(surviveTime >= 5f && surviveTime < 10f && spawnerCounter == 1)
            {
                GameObject bulletSpawner = Instantiate(bullerSpawnerPrefab, bulletSpawners[spawnerCounter], Quaternion.identity);
                bulletSpawner.transform.parent = level.transform;
                bulletSpawner.transform.localPosition = bulletSpawners[spawnerCounter];
            ...
            else if (surviveTime >= 10f && surviveTime < 15f && spawnerCounter == 2)
            {
                GameObject bulletSpawner = Instantiate(bullerSpawnerPrefab, bulletSpawners[spawnerCounter], Quaternion.identity);
                bulletSpawner.transform.parent = level.transform;
                bulletSpawner.transform.localPosition = bulletSpawners[spawnerCounter];
            ...
            else if (surviveTime >= 15f && surviveTime < 20f && spawnerCounter == 3)
            {
                GameObject bulletSpawner = Instantiate(bullerSpawnerPrefab, bulletSpawners[spawnerCounter], Quaternion.identity);
                bulletSpawner.transform.parent = level.transform;
                bulletSpawner.transform.localPosition = bulletSpawners[spawnerCounter];
